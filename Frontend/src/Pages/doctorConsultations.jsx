...
export default function DoctorConsultations() {
  ...
  // Auto-fetch patient animal medical history whenever a consultation detail is selected
  useEffect(() => {
    const animalId = selectedRequestDetails?.animal_id || selectedRequestDetails?.animalId;
    if (animalId) {
      setLoadingHistory(true);
      const token = localStorage.getItem("token") || "";
      axios.get(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000'}/api/animals/${animalId}/history`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(res => setSelectedAnimalHistory(res.data || []))
        .catch(err => console.error("Failed to fetch animal history:", err))
  ...
  const extractSymptoms = (notes) => {
    ...
    let cleanText = String(notes).trim();
    if (cleanText.includes('{') && cleanText.includes('}')) {
      try {
        const jsonSub = cleanText.substring(cleanText.indexOf('{'), cleanText.lastIndexOf('}') + 1);
        const parsed = JSON.parse(jsonSub);
        cleanText = parsed.symptoms || parsed.notes || parsed.reason || "";
      } catch {
        // ignore
      }
    }
    ...
    if (!lower || lower.includes('no symptom') || lower === 'none' || lower === 'n/a') {
      return [];
    }
    ...
    if (lower.includes('vomit') || lower.includes('nausea') || lower.includes('throw up')) symptomsList.push('Vomiting');
    if (lower.includes('diarrhea') || lower.includes('stool') || lower.includes('bowel')) symptomsList.push('Diarrhea');
    if (lower.includes('limp') || lower.includes('lame') || lower.includes('walking')) symptomsList.push('Lameness / Limping');
    if (lower.includes('skin') || lower.includes('rash') || lower.includes('itch') || lower.includes('lesion')) symptomsList.push('Skin Lesions / Itching');

    if (symptomsList.length === 0 && cleanText.length > 0) {
      const parts = cleanText.split(/[,.]+/).map(w => w.trim()).filter(w => w.length > 0 && w.length < 30);
      if (parts.length > 0 && !parts[0].toLowerCase().includes('no symptom')) {
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card className="p-6 border-slate-200 shadow-sm space-y-6 bg-white rounded-2xl">
                ...
                {/* Diagnostic Symptoms & Case Details */}
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    {symptoms.length > 0 ? (
                      symptoms.map((sym, idx) => (
                        <Badge key={idx} className="bg-amber-100/90 text-amber-900 border-amber-200/90 font-bold px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5 shadow-2xs">
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                          {sym}
                        </Badge>
                      ))
                    ) : (
                      <span className="text-sm text-slate-500 font-medium italic bg-white px-3.5 py-1.5 rounded-xl border border-slate-200 inline-block shadow-2xs">
                ...
                {(request.animal_health_report || request.health_report || request.originalData?.animal_health_report || request.originalData?.health_report) && (
                  <div className="bg-emerald-50/90 border border-emerald-200 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium">Uploaded record for patient {request.animal_name || 'Patient'}</p>
                      </div>
                    </div>
                    <a
                      href={getFileUrl(request.animal_health_report || request.health_report || request.originalData?.animal_health_report || request.originalData?.health_report)}
                      target="_blank"
                ...
                {/* Patient Medical History Button */}
                <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
                  <span className="text-xs text-slate-500 font-medium">Patient Clinical History</span>
                  <button
                    type="button"
                    onClick={() => setViewingHistoryModal(true)}
                  ...
                  >
                    <FileText size={16} />
                    View Full Medical Record History
                  </button>
                </div>
              ...
              <Card className="p-6 border-slate-200 shadow-sm bg-white rounded-2xl space-y-4">
                ...
                {loadingHistory ? (
                  ...
                  <div className="p-6 bg-slate-50 border border-slate-100 rounded-xl text-center text-xs text-slate-500 font-medium">
                    No medical history records or past consultations logged for this animal yet.
                  </div>
                )}
              </Card>
      ...
      {viewingHistoryModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-slate-800">
            {/* Modal Header */}
